...
def load_config(config_path: Path | None, cli_overrides: dict[str, Any]) -> Config:
    ...
    config = Config(
        ...
        keep_alive=str(merged.get("keep_alive", "20m")),
        ...
        medium_confidence=float(merged.get("medium_confidence", 0.78)),
        min_topic_size=int(merged.get("min_topic_size", 3)),
        min_subtopic_size=int(merged.get("min_subtopic_size", 3)),
        prompt_version=str(merged.get("prompt_version", "v3")),
        schema_version=str(merged.get("schema_version", "v3")),
        cache_enabled=bool(merged.get("cache_enabled", True)),
        ...
        review_bucket=str(merged.get("review_bucket", "misc_topics")),
        ...
        rename_trace_file=_as_path(merged.get("rename_trace_file")),
        manifest_dir=_as_path(merged.get("manifest_dir")),
        log_dir=_as_path(merged.get("log_dir")),
        trash_empty_dir_name=str(merged.get("trash_empty_dir_name", ".trash_empty_dirs")),
        garbage_collect_empty_dirs=bool(merged.get("garbage_collect_empty_dirs", True)),
        max_light_chars=int(merged.get("max_light_chars", 1200)),
        max_heavy_chars=int(merged.get("max_heavy_chars", 3500)),
    )
